...
const OrderbookViewer = ({ venue, symbol, simulatedOrder }: OrderbookViewerProps) => {
  ...
  if (orderbook.bids.length === 0 && orderbook.asks.length === 0) {
    return (
      <div className="orderbook-viewer p-4 bg-gradient-to-br from-gray-900 to-black border border-white/20 rounded-xl shadow-2xl">
          ...
          <div className="animate-pulse">
            {!wsState.connectionError ? (
              <>
                ...
                <p className="text-white/80 font-medium">Connecting to {VENUES.find(v => v.value === venue)?.label}...</p>
                <p className="text-white/60 text-sm mt-2">Establishing real-time WebSocket connection</p>
              ...
              <>
                ...
                <p className="text-white/60 text-sm">Unable to connect to {VENUES.find(v => v.value === venue)?.label}</p>
                <div className="mt-4 p-3 bg-red-500/10 border border-red-400/30 rounded-lg text-sm">
                  ...
                  <p className="text-white/70">{wsState.connectionError}</p>
                </div>
                {wsState.reconnectAttempts >= 3 && (
                  <button
                    onClick={manualReconnect}
                    className="mt-4 px-4 py-2 bg-blue-500/20 border border-blue-400/50 text-blue-300 rounded-lg hover:bg-blue-500/30 transition"
                  >
                    Try Again
                  </button>
                )}
  ...
  return (
    <div className="orderbook-viewer p-4 bg-gradient-to-br from-gray-900 to-black border border-white/20 rounded-xl shadow-2xl">
      ...
      {orderMetrics && (
        <div className="bg-gradient-to-r from-blue-900/20 to-purple-900/20 border border-white/20 rounded-xl p-6">
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-6 mb-4">
            <div className="text-center">
              <div className="text-white/60 text-sm mb-1">Fill Percentage</div>
            ...
            <div className="text-center">
              <div className="text-white/60 text-sm mb-1">Market Impact</div>
              <div className={`font-mono text-2xl font-bold ${
            ...
            <div className="text-center">
              <div className="text-white/60 text-sm mb-1">Slippage</div>
              <div className={`font-mono text-2xl font-bold ${
            ...
            <div className="text-center">
              <div className="text-white/60 text-sm mb-1">Est. Time to Fill</div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4 text-sm">
            <div>
              ...
              <div className="text-white font-mono text-lg">{formatCurrency(orderMetrics.averageFillPrice)}</div>
            </div>
            <div>
              <div className="text-white/60">Total Cost:</div>
              <div className="text-white font-mono text-lg">{formatCurrency(orderMetrics.totalCost)}</div>
            </div>
          </div>
            ...
            <div className="mt-4 space-y-2">
              {orderMetrics.liquidityWarning && (
                <div className="flex items-center gap-2 text-yellow-400 bg-yellow-400/10 rounded-lg p-3">
                  ...
                  <span className="font-semibold">Liquidity Warning: Order may not be fully filled at current market depth.</span>
                </div>
              )}
              {orderMetrics.priceImpactWarning && (
                <div className="flex items-center gap-2 text-red-400 bg-red-400/10 rounded-lg p-3">
                  <span className="text-lg">🚨</span>
                  <span className="font-semibold">High Impact Warning: This order may significantly affect market price.</span>
                </div>
              )}
            </div>
          )}
